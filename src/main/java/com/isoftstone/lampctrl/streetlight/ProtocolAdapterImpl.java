package com.isoftstone.lampctrl.streetlight;
...
import com.isoftstone.codec.HuatiCloud2Machine;
import com.isoftstone.codec.HuatiMachine2Cloud;
...
public class ProtocolAdapterImpl implements IProtocolAdapter {
    ...
    public byte[] encode(ObjectNode input) throws Exception {
        try {
            AbstractCloud2Machine msg = HuatiCloud2Machine.getInstance(input);
            return msg.handle();
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public ObjectNode decode(byte[] binaryData) throws Exception {
        try {
            AbstractMachine2Cloud data = HuatiMachine2Cloud.getInstance(binaryData);
            return data.handle();
        } catch (Exception e) {
            e.printStackTrace();
            return null;
